...
def fractal_opencl(q, frcl_pars):
        ...
        for(int curiter = 0; curiter < maxiter; curiter++) {
            ...
            if (real2 + imag2 > 4.0f){
                if (reverse > 0)
                    ...
                    output[gid] = maxiter-curiter;
                 return;
            }            
            
            imag = 2.0f * real*imag + ci;
            real = real2 - imag2 + cr;
        }
    }
    """).build()
    ...
    prg.fractal(queue, output.shape, 
                None, 
                q_opencl, 
                output_opencl, 
                np.uint16(frcl_pars.maxiter),
                np.uint16(frcl_pars.reverse),
...
from PyQt5.QtWidgets import QWidget, QApplication, QMainWindow, QLabel, QHBoxLayout, QFileDialog
from PyQt5.QtGui import QImage, QPixmap,  QTransform, QWheelEvent, QKeyEvent, QMouseEvent
import sys
...
class FractalWidget(QWidget):
    ...
    def update_fractal(self):
        ...
        x,y,z = fractal_set(x_min, x_max, y_min, y_max, self.imageLabel.width(), self.imageLabel.height(), self.frcl_pars)
        zz = z.ravel().astype(np.uint8)
        img = QImage(zz.data, self.imageLabel.width(), self.imageLabel.height(), self.imageLabel.width(), QImage.Format_Indexed8)
    ...
    def mousePressEvent(self, event):
        ...
        super(FractalWidget, self).mousePressEvent(event)
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == QtCore.Qt.Key_I:
            self.frcl_pars.reverse = 1 - self.frcl_pars.reverse
            self.update_fractal()
...
class FractalWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.fractalWidget = FractalWidget(width, height, self.statusBar())
